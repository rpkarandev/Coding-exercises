...
void msdsample(int delt, int stime, int etime)
{
	...
	absconfig(IntStr, delt, stime, etime);
	
	cout<<"Computing <Mean Square Displacement> for "<<N<<" atoms \n";	
	sprintf( IntStr, "absconfig%d.xyz",ind);
   	ifstream file(IntStr,std::ifstream::binary);	
		for(dt = delt; dt <= time/2; (dt = dt + delt),timesteps++) {		//cout<<"dt == "<<dt<<"\n";		
		    nbins = 0;
			for(int t0 = 0; t0 < (time - dt); (t0 = t0 + dt),nbins++) {	//cout<<"t == "<<t0<<"\t";
...
void absconfig(char* outputfilename,int delt, int stime, int etime) {
	...
	for(int t = stime; t <= etime; t++)	{
		...
		if(t > stime) {
			...
			}copycoordinates(&rabs[0], &rprev[0]);		   
		}else { 
...
std::ifstream& readconf(std::ifstream& file,bool abs ,unsigned int time, Cordinates *r){
	...
	// skiping time
	for(int j = 0; j < time; j++) { file>>str; 	for(int i=0; i < N; i++){ file>>str>>num>>num>>num; } 	}
	//reading data for time "time"
	file>>str; 	for(int i = 0; i < N; i++) { file >> str >> r[i].x >> r[i].y >> r[i].z;	}
...
void rdfsample()
{
double delr = box/double(2*maxbin);
double pi = 3.1415926;
